...
struct CD3DX12_RESOURCE_BARRIER : public D3D12_RESOURCE_BARRIER {
  ...
  static inline CD3DX12_RESOURCE_BARRIER
  UAV(_In_ ID3D12Resource* pResource) noexcept {
    CD3DX12_RESOURCE_BARRIER result = {};
    D3D12_RESOURCE_BARRIER& barrier = result;
...
struct CD3DX12_ROOT_PARAMETER : public D3D12_ROOT_PARAMETER {
  ...
  static inline void
  InitAsConstantBufferView(_Out_ D3D12_ROOT_PARAMETER& rootParam,
  ...
  static inline void
  InitAsShaderResourceView(_Out_ D3D12_ROOT_PARAMETER& rootParam,
                           UINT shaderRegister, UINT registerSpace = 0,
                           D3D12_SHADER_VISIBILITY visibility =
                               D3D12_SHADER_VISIBILITY_ALL) noexcept {
  ...
  static inline void
  InitAsUnorderedAccessView(_Out_ D3D12_ROOT_PARAMETER& rootParam,
                            UINT shaderRegister, UINT registerSpace = 0,
                            D3D12_SHADER_VISIBILITY visibility =
                                D3D12_SHADER_VISIBILITY_ALL) noexcept {
  ...
  inline void InitAsConstants(UINT num32BitValues, UINT shaderRegister,
                              UINT registerSpace = 0,
                              D3D12_SHADER_VISIBILITY visibility =
                                  D3D12_SHADER_VISIBILITY_ALL) noexcept {
  ...
  inline void
  InitAsShaderResourceView(UINT shaderRegister, UINT registerSpace = 0,
                           D3D12_SHADER_VISIBILITY visibility =
                               D3D12_SHADER_VISIBILITY_ALL) noexcept {
  ...
  inline void
  InitAsUnorderedAccessView(UINT shaderRegister, UINT registerSpace = 0,
                            D3D12_SHADER_VISIBILITY visibility =
                                D3D12_SHADER_VISIBILITY_ALL) noexcept {
...
struct CD3DX12_ROOT_DESCRIPTOR1 : public D3D12_ROOT_DESCRIPTOR1 {
  ...
  static inline void Init(_Out_ D3D12_ROOT_DESCRIPTOR1& table,
                          UINT shaderRegister, UINT registerSpace = 0,
                          D3D12_ROOT_DESCRIPTOR_FLAGS flags =
                              D3D12_ROOT_DESCRIPTOR_FLAG_NONE) noexcept {
    table.ShaderRegister = shaderRegister;
...
struct CD3DX12_ROOT_PARAMETER1 : public D3D12_ROOT_PARAMETER1 {
  ...
  static inline void InitAsConstants(_Out_ D3D12_ROOT_PARAMETER1& rootParam,
                                     UINT num32BitValues, UINT shaderRegister,
                                     UINT registerSpace = 0,
                                     D3D12_SHADER_VISIBILITY visibility =
                                         D3D12_SHADER_VISIBILITY_ALL) noexcept {
...
struct CD3DX12_CPU_DESCRIPTOR_HANDLE : public D3D12_CPU_DESCRIPTOR_HANDLE {
  ...
  Offset(INT offsetScaledByIncrementSize) noexcept {
    ptr = SIZE_T(INT64(ptr) + INT64(offsetScaledByIncrementSize));
    return *this;
  }
  bool
  operator==(_In_ const D3D12_CPU_DESCRIPTOR_HANDLE& other) const noexcept {
  ...
  CD3DX12_CPU_DESCRIPTOR_HANDLE&
  operator=(const D3D12_CPU_DESCRIPTOR_HANDLE& other) noexcept {
    ptr = other.ptr;
    return *this;
  }
...
struct CD3DX12_GPU_DESCRIPTOR_HANDLE : public D3D12_GPU_DESCRIPTOR_HANDLE {
  ...
  inline bool
  operator==(_In_ const D3D12_GPU_DESCRIPTOR_HANDLE& other) const noexcept {
    return (ptr == other.ptr);
  }
  inline bool
  operator!=(_In_ const D3D12_GPU_DESCRIPTOR_HANDLE& other) const noexcept {
  ...
  CD3DX12_GPU_DESCRIPTOR_HANDLE&
  operator=(const D3D12_GPU_DESCRIPTOR_HANDLE& other) noexcept {
    ptr = other.ptr;
    return *this;
  }
...
//------------------------------------------------------------------------------------------------
inline constexpr UINT D3D12CalcSubresource(UINT MipSlice, UINT ArraySlice,
...
struct CD3DX12_RESOURCE_DESC : public D3D12_RESOURCE_DESC {
  ...
  inline UINT CalcSubresource(UINT MipSlice, UINT ArraySlice,
...
inline void MemcpySubresource(_In_ const D3D12_MEMCPY_DEST* pDest,
  ...
  for (UINT z = 0; z < NumSlices; ++z) {
    ...
    auto pSrcSlice =
        static_cast<const BYTE*>(pSrc->pData) + pSrc->SlicePitch * LONG_PTR(z);
...
inline void MemcpySubresource(_In_ const D3D12_MEMCPY_DEST* pDest,
  ...
  for (UINT z = 0; z < NumSlices; ++z) {
    ...
    auto pSrcSlice = (static_cast<const BYTE*>(pResourceData) + pSrc->Offset) +
                     pSrc->DepthPitch * ULONG_PTR(z);
    for (UINT y = 0; y < NumRows; ++y) {
      memcpy(pDestSlice + pDest->RowPitch * y,
             pSrcSlice + pSrc->RowPitch * ULONG_PTR(y), RowSizeInBytes);
    }
  }
}
...
D3D12IsLayoutOpaque(D3D12_TEXTURE_LAYOUT Layout) noexcept {
...
inline ID3D12CommandList* const*
CommandListCast(t_CommandListType* const* pp) noexcept {
...
public:
  ...
  CD3DX12_PIPELINE_STATE_STREAM_SUBOBJECT&
  operator=(InnerStructType const& i) noexcept {
    _Type = Type;
    _Inner = i;
    return *this;
  }
...
struct CD3DX12_PIPELINE_MESH_STATE_STREAM {
  ...
  noexcept
      : Flags(Desc.Flags), NodeMask(Desc.NodeMask),
        pRootSignature(Desc.pRootSignature), PS(Desc.PS), AS(Desc.AS),
  ...
  for (SIZE_T CurOffset = 0, SizeOfSubobject = 0; CurOffset < Desc.SizeInBytes;
    ...
    case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_STREAM_OUTPUT:
      pCallbacks->StreamOutputCb(
          *reinterpret_cast<
              decltype(CD3DX12_PIPELINE_STATE_STREAM::StreamOutput)*>(pStream));
...
#define D3DX12_COM_PTR winrt::com_ptr
#define D3DX12_COM_PTR_GET(x) x.get()
#define D3DX12_COM_PTR_ADDRESSOF(x) x.put()
...
public:
  ...
  void SetExistingCollection(ID3D12StateObject* pExistingCollection) noexcept {
    ...
    m_CollectionRef.attach(pExistingCollection);
  }
  void DefineExport(LPCWSTR Name, LPCWSTR ExportToRename = nullptr,
                    D3D12_EXPORT_FLAGS Flags = D3D12_EXPORT_FLAG_NONE) {
    D3D12_EXPORT_DESC Export;
