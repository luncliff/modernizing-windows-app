...
HRESULT DXGIProvider::create_device(ID3D12Device** device,
                                    bool hardware) noexcept(false) {
  try {
    if (hardware)
      GetHardwareAdapter(factory.get(), adapter.put());
    else if (auto hr = factory->EnumWarpAdapter(__uuidof(IDXGIAdapter1),
                                                adapter.put_void());
             FAILED(hr)) {
...
HRESULT
DXGIProvider::create_swapchain(const DXGI_SWAP_CHAIN_DESC1& desc,
                               ID3D11Device* device,
                               IDXGISwapChain1** output) noexcept(false) {
  if (auto hr = factory->CreateSwapChainForComposition(device, &desc, nullptr,
                                                       output);
      FAILED(hr))
    winrt::throw_hresult(hr);
...
#endif
  if (auto hr = CreateDXGIFactory2(dxgi_flags, __uuidof(IDXGIFactory4),
                                   factory.put_void());
      FAILED(hr))
    winrt::throw_hresult(hr);
}
