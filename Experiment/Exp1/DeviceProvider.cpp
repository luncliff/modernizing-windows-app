...
DeviceProvider::DeviceProvider(DXGIProvider& dxgi) noexcept(false) {
  UINT d11_flags = D3D11_CREATE_DEVICE_BGRA_SUPPORT | D3D11_CREATE_DEVICE_VIDEO_SUPPORT;
#if defined(_DEBUG)
  if (auto hr = D3D12GetDebugInterface(__uuidof(ID3D12Debug), debug_control.put_void()); SUCCEEDED(hr)) {
  ...
  if (FAILED(dxgi.create_device(d12_device.put(), true))) {
    ...
    if (auto hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, NULL, d11_flags, levels, 1, D3D11_SDK_VERSION,
                                    d11_device.put(), &level, d11_device_context.put());
        FAILED(hr))
    ...
    // DX11 device on the DX12 device, command queue
    {
      ...
      if (auto hr = D3D11On12CreateDevice(d12_device.get(), d11_flags, nullptr, 0, &cq, 1, 0, d11_device.put(),
                                          d11_device_context.put(), nullptr);
          FAILED(hr))
        winrt::throw_hresult(hr);
      // query the 11on12 device
      if (auto hr = d11_device->QueryInterface(d11_on_d12_device.put()); FAILED(hr))
        winrt::throw_hresult(hr);
    }
#if defined(_DEBUG)
    if (auto hr = d12_device->QueryInterface(d11_on_d12_info.put()); SUCCEEDED(hr)) {
...
winrt::com_ptr<ID3D12CommandQueue> DeviceProvider::get_dx12_command_queue() noexcept {
...
winrt::com_ptr<ID3D11DeviceContext> DeviceProvider::get_dx11_device_context() noexcept {
